...
For our synthetic dataset the time difference between the two methods is large. The initial preprocessing steps where the matrix is read and scaled contribute very little to any of the methods. For FlashPCA, the running time consists of two major components which is an initial matrix multiply, $XX'$, and the subsequent subspace iterations. Forming the matrix $XX'$ is the default option in FlashPCA and might be a relict from the time when the number of gene positions (columns) was much larger the number of sample (rows). For and aspect ration of $\frac{1}{2}$ this computation is expensive. In addition to the pure flop expense in computing $XX'$, on our system, there is a large penalty for using Eigen relative to MKL where the former executes in 926 and the latter in 306 seconds for the same number of threads. In contrast to the algorithmic differences, this difference might be very system dependent.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{semilogyaxis}[width=10cm, height=5cm]
            \addplot[very thick] file {data/condNumberNorm.txt};
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{Condition number of the matrix of subspace basis per iteration for FlashPCA without QR}
\end{figure}
