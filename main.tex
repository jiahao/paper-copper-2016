...
\section{Confounding factors in GWAS studies}
...
There are three major confounding sources of variation which are considered in
the analysis of human genome data:

\paragraph{Population stratification}
In linear algebraic terms, the genomics matrix will have a low rank component
with large singular values.

\paragraph{Kinship}
Kinship, or local ancestry, consists of
The genomics matrix is expected to have (near) linear dependencies in its columns,
...
LD is usually treated with a separate preprocessing step to correct for false
positives in downstream statistical analyses~\cite{Sankararaman2008}.
...
\begin{equation}
%    \label{eq:mplaw-sv}
    p_s(x) = \frac {\sqrt{(\sigma_+^2-x^2)(x^2-\sigma_-^2)}} {\pi \sigma \min(1, \lambda) x}
\end{equation}
%
where
